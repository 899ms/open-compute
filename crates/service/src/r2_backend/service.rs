...
pub struct R2BindingService {
    ...
    pub(super) staging: R2Staging,
    pub(super) uploads: OperationGate,
    pub(super) downloads: OperationGate,
    pub(super) staging_bytes: Arc<AtomicU64>,
    pub(super) checksums: Arc<tokio::sync::Semaphore>,
...
impl R2BindingService {
    ...
    async fn handle_inner(
        ...
        let result = match operation {
            ...
            Operation::Delete => {
                self.delete_objects(&binding, &locator, request.into_body(), timeout)
                    .await?
            }
            Operation::List => {
                let body = bounded_json(request.into_body(), MAX_METADATA_BYTES).await?;
    ...
    async fn delete_objects(
        ...
        let body = bounded_json(body, MAX_DELETE_BODY_BYTES).await?;
