...
export class AiSearchTransport extends WorkerEntrypoint<
  BindingEnv,
  ResourceBindingProps & { instance?: string }
> {
  #headers(contentType: string): Record<string, string> {
    ...
    return {
      ...
      "x-open-compute-resource-generation": String(
        props.resourceSpecGeneration,
      ),
  ...
  get instance(): string | undefined {
    return this.ctx.props?.instance;
  }
  async #fetch(
    path: string,
    init: RequestInit,
    ...
    const response = await this.env.BINDING_BACKEND.fetch(
      ...
      { ...init, headers: this.#headers(contentType) },
    );
    if (!response.ok)
      throw bindingError(
  ...
  async call(
    ...
    const response = await this.#fetch("/internal/ai-search/v1/call", {
      method: "POST",
      body: JSON.stringify({ operation, instance, payload }),
    });
    await expectBindingStatus(response, 200, "AI_SEARCH_PROTOCOL_ERROR");
    const value = await bindingJson(response, "AI_SEARCH_PROTOCOL_ERROR");
    if (
      !isRecord(value) ||
  ...
  async stream(
    ...
    if (
      !["namespace.chatCompletions", "instance.chatCompletions"].includes(
        operation,
      )
    )
      throw bindingError("AI_SEARCH_PROTOCOL_ERROR");
    const response = await this.#fetch("/internal/ai-search/v1/stream", {
      method: "POST",
      body: JSON.stringify({ operation, instance, payload }),
    });
    await expectBindingStatus(response, 200, "AI_SEARCH_PROTOCOL_ERROR");
    if (response.body === null) throw bindingError("AI_SEARCH_PROTOCOL_ERROR");
    return response;
  }
  async upload(
    instance: string | undefined,
    ...
    const response = await this.#fetch(
      "/internal/ai-search/v1/upload",
      {
        method: "POST",
        body: framed(
          { schemaVersion: 1, instance, name, contentType, options },
          body,
          "AI_SEARCH_LIMIT_EXCEEDED",
        ),
      },
      "application/vnd.open-compute.ai-search.v1+frame",
    );
    await expectBindingStatus(response, 200, "AI_SEARCH_PROTOCOL_ERROR");
    const value = await bindingJson(response, "AI_SEARCH_PROTOCOL_ERROR");
    if (
  ...
  download(instance: string | undefined, itemId: string): Promise<Response> {
    return this.#fetch("/internal/ai-search/v1/download", {
      method: "POST",
      body: JSON.stringify({ instance, itemId }),
    });
  }
}
