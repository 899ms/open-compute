...
| [P18 单域名公网网关、DNS 与 TLS](p18-single-domain-public-gateway.md)              | Day 1 单基础域名、Worker 一 local/至多一 public 与 P17 常驻 child 复用合同已同步；待实现 migration/resolver/endpoint、Caddy 接入及 DNS/ACME 验收；不新增全局 Coordinator                                             |
