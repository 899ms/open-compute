...
| [P16 capability-scoped TypeScript SDK](p16-capability-scoped-typescript-sdk.md)    | 已实现并关闭 #68：`packages/sdk` 只暴露 qualified Cloudflare subset 与 `client.openCompute`；首包由 0.1.9 release workflow 通过已授权的 `NPM_ACCESS_TOKEN` 发布并回读验证                                              |
