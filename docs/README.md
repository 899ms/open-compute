...
| [P17 宿主子进程管理基础设施](p17-host-process-infrastructure.md)                   | Day 1 三层架构完成：通用 Host Process Runtime、composition-root Process Coordinator、独立 Domain Managers；待抽取现有 workerd/Xberg process ownership，并供 Extension Provider 与 Browser 复用 |
| [P18 单域名公网网关、DNS 与 TLS](p18-single-domain-public-gateway.md)              | Day 1 单基础域名、固定产品 namespace、手工 wildcard/challenge DNS、内嵌 pinned Caddy、自有 DNS-01 provider 与 `ocd` Host 路由合同完成；待实现 Caddy supply chain、challenge DNS、持久化 authority、Worker/R2 公网入口与真实 DNS/ACME 验收 |
