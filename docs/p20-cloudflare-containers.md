# P20：Cloudflare Containers 兼容设计
...
中的 Container upload、Containers control API 和 Durable Object Container runtime。P20 以固定 Cloudflare 公共合同、
...
P20 与 [P19 Browser Run](p19-browser-run.md) 共享单 executable 分发目标以及外部／嵌入式 runtime 的 ownership 边界：
...
context 上的 native capability，包含 Fetcher、Socket、streaming exec、monitor 和 lifecycle identity。P20 不用普通 HTTP
...
- 不保留 Wrangler 旧 `containers.configuration` 兼容路径；P20 只实现固定 Day 1 schema。
...
P20 实施时直接修改当前 Day 1 validator/schema/fixtures，不保留“旧版本拒绝、新版本接受”的历史分支。
...
Worker code可能在 rollout 完成前访问旧 image instance。P20 因此必须持久化：
...
共享其 network namespace。P20 保留一个 authoritative egress path：
...
容器部署自身平台组件，不作为 P20 provider或API authority。后续发现的 WDL功能只有通过同一官方合同/测试后才可引用，不能因
...
Exit：若 native capability不能安全挂到 dynamic loaded DO/facet，P20保持 unsupported；不能退回JS模拟或把所有tenant共用一个
