...
状态：**W1 与 W2 均已完成；W3 用户可扩展原生 Binding 已更新静态配置与生命周期设计，G0 与实现尚未完成**。W2 的 Wrangler/v4 配置、Dynamic Worker ceiling、原生执行、公开错误、
...
| [W3 用户可扩展原生 Binding](w3-user-extensible-native-bindings.md)                       | `ocd` 静态配置、Binding 隔离、Provider 生命周期、Broker/直连数据面与 G0 |
