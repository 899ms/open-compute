...
状态：**W1 与 W2 均已完成；W3 用户可扩展原生 Binding 已收敛为本地加载器与 services/props 绑定，G0 与实现尚未完成**。W2 的 Wrangler/v4 配置、Dynamic Worker ceiling、原生执行、公开错误、
...
| [W3 用户可扩展原生 Binding](w3-user-extensible-native-bindings.md)                       | `ocd` 名字/path 注册、Wrangler `services + props`、Provider 直连与生命周期；不管理扩展版本 |
