# P21：macOS Developer ID 签名与 Apple 公证

状态：Day 1 发行合同与 CI 方案完成；待配置 Apple/GitHub 凭据、实施和真实 tag 验收。

P21 为正式 `darwin-arm64` 单文件 `ocd` 接入 Apple Developer ID 签名与 Notary Service 公证。目标是让从
...
P21 只使用 `notarytool`，不保留已退役的 `altool` 路径。实现必须在 job 开始时记录无 secret 的 macOS build、
...
下载的 CER 不能完成签名。P21 不创建 `Developer ID Installer` certificate，因为当前产品不发布 PKG。
...
P21 选择 App Store Connect API Key，不实现 Apple ID + app-specific password 的第二套 CI 路径。Team API Key 按 Apple
...
并为未签名 candidate 生成 package report。P21 保留这一步作为 candidate provenance，但它不再是公开产物。
...
`ocd` 当前不需要 entitlement；若 Hardened Runtime 暴露真实不兼容，先修复代码或把精确 capability 与风险写入 P21 review，
不能加一组宽泛 entitlement。
...
package，不能把 ticket staple 到当前裸 executable 或一次性 ZIP。因此 P21 的 Gatekeeper 验证使用 Apple 在线 ticket；
默认 GitHub 安装流程本来就需要联网下载 release。`ocd` 启动后仍不联系 Apple，production offline contract 不变。

完全离线的首次 Gatekeeper admission 不属于 P21 当前范围。若将来要求在从未访问 Apple 的隔离 Mac 上完成首次安装，应另行
评估发布 stapled PKG/DMG；这会改变“五个 asset、无 installer”的正式发行合同，不能在 P21 中偷偷增加第六个 artifact。
...
P21 第一阶段只签最终公开 `ocd`，不在 release CI 中临时重签 `share/workerd/darwin-arm64/workerd`：
...
摘要与跨平台 Gate。不得在 P21 job 中重签后跳过 formal pin。

## 7. Release manifest 与流水线所有权

公开 `release.json` 的现有 schema 已能用 target、size 和 SHA-256 唯一指向签名后的 macOS bytes，无需加入可从 Mach-O
signature 读取的重复字段，也无需为 P21 修改 public schema。Apple signing metadata 只进入新的 current final package
...
首个 P21 tag 必须在 `macos-15` runner 上取得并保留以下证据：
...
- P21 完成后删除凭据配置过程和实施步骤，只把当前发行合同与实际证据精简移入 `docs/implemented/`。
