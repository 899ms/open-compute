# P19：Cloudflare Browser Run 兼容设计
...
仍为 Beta，且官方文档存在 Workers/browser service 约束。P19 不把整份 Chrome CDP schema 自动宣布为支持合同：正式范围
...
- 当前目标没有已验证的内嵌 Browser Runtime，或 P19 capability 未通过时，upload fail closed，不能删除 binding 后继续部署。
...
deployment-owned frontend。P19 不在启动时从公网下载 DevTools UI。若不能合法、可复现地随 release 提供兼容 frontend，
...
`wrangler dev` 的本地体验继续由上游 Wrangler/Miniflare 负责；P19 qualification 针对真实单文件 `ocd` + stock workerd +
