...
function WorkflowDetailPage() {
  ...
  const [confirmedAction, setConfirmedAction] = useState<Extract<
    WorkflowAction,
    "terminate" | "restart"
  > | null>(null);
  ...
  const refreshRuntime = async () => {
    await Promise.all([
      instances.refetch(),
      selectedInstanceId === null ? Promise.resolve() : instance.refetch(),
    ]);
  };
    ...
    onError: (error) => {
      setMutationError(
        error instanceof Error
          ? error.message
          : "Unable to update the workflow definition.",
      );
  ...
  const actionMutation = useMutation({
    ...
    onError: (error) => {
      setMutationError(
        error instanceof Error
          ? error.message
          : "Unable to update the workflow instance.",
      );
  ...
  const eventMutation = useMutation({
    ...
    onError: (error) => {
      setMutationError(
        error instanceof Error
          ? error.message
          : "Unable to send the workflow event.",
      );
  ...
  return (
    <div>
      ...
      <ConfirmActionDialog
        title={
          confirmedAction === "terminate"
            ? "Terminate workflow instance"
            : "Restart workflow instance"
        }
        description={
          confirmedAction === "terminate"
            ? "Termination stops this instance."
            : "Restart the selected workflow instance."
        }
        ...
        submitVariant={
          confirmedAction === "terminate" ? "destructive" : "primary"
        }
        ...
        onConfirm={() => {
          if (confirmedAction) actionMutation.mutate(confirmedAction);
        }}
      />
      {workflow.isLoading || versions.isLoading || instances.isLoading ? (
        <LoadingState />
      ) : workflow.error || versions.error || instances.error ? (
        <ErrorState message="Unable to load Workflow details." />
      ) : (
        <>
          ...
          {selectedInstanceId ? (
            <div className="mt-6">
              ...
              {instance.isLoading ? (
                ...
                <ErrorState message="Unable to load the selected instance." />
              ) : (
                <>
                  <div className="mb-4 flex flex-wrap gap-2">
                    ...
                    <Button
                      variant="secondary"
                      disabled={actionMutation.isPending}
                      onClick={() => setConfirmedAction("restart")}
                    >
                      Restart
                    </Button>
                    <Button
                      ...
                      onClick={() => setConfirmedAction("terminate")}
                    >
                      Terminate
                    </Button>
                  </div>
                  <DataTable
                    ...
                    rows={(instance.data?.steps ?? []).map((step) => ({
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_2fr_auto] sm:items-end">
                    ...
                    <Button
                      variant="primary"
                      disabled={eventMutation.isPending || !eventType.trim()}
