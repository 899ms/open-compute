...
function D1DetailPage() {
  ...
  const query = useMutation({
    ...
    onError: (error) =>
      feedback.failure(error, "Unable to execute the D1 query."),
  });
  const createBackup = useMutation({
    ...
    onError: (error) =>
      feedback.failure(error, "Unable to create the D1 backup."),
  });
    ...
    onError: (error) => {
      setMutationError(
        error instanceof Error
          ? error.message
          : "Unable to restore the D1 backup.",
      );
  ...
  return (
    <div>
      ...
      <CreateResourceDialog
        ...
        description="Create a new D1 database from the selected backup."
      ...
      />
      {database.isLoading || backups.isLoading ? (
        <LoadingState />
      ) : database.error || backups.error ? (
        <ErrorState message="Unable to load D1 database details." />
      ) : (
        <>
          <DataTable
            columns={[
            ...
            rows={[
              {
                property: "Created",
                value: database.data?.created_at ?? "unknown",
              },
              {
                property: "Tables",
                value: database.data?.num_tables ?? "unknown",
              },
              {
                property: "File size",
                value: database.data?.file_size ?? "unknown",
              },
              {
                property: "Jurisdiction",
                value: database.data?.jurisdiction ?? "default",
              },
              {
                property: "Read replication",
                value: database.data?.read_replication?.mode ?? "unknown",
              },
            ]}
          />
          <div className="mt-6">
            <SectionHeader
              title="Query"
              description="Execute SQL through the official D1 query endpoint."
            />
            <textarea
              aria-label="SQL query"
              className="min-h-36 w-full rounded border p-3 font-mono text-sm"
              value={sql}
            ...
            {query.error ? (
              <div className="mt-3">
                <ErrorState message="Query failed." />
              </div>
            ) : null}
            {query.data ? (
              <pre className="bg-kumo-tinted mt-3 max-h-96 overflow-auto rounded p-4 text-sm whitespace-pre-wrap">
