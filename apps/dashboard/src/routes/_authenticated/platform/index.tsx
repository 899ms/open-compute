  ...
  return (
    <div>
      ...
      {status.isLoading ? (
        ...
        <ErrorState message="Unable to load platform status." />
      ) : (
        <DataTable
          columns={[
          ...
          rows={[
            {
              name: "Scheduler",
              detail: `${scheduler?.state ?? "unknown"}; ${scheduler?.pending ?? 0} pending; ${scheduler?.running ?? 0} running`,
            },
            {
              name: "Cache",
              detail: `${status.data?.[1].entries ?? 0} entries; ${status.data?.[1].bytes ?? 0} bytes`,
            },
            {
              name: "Images",
              detail: `${status.data?.[2].running ?? 0}/${status.data?.[2].capacity ?? 0} running; ${status.data?.[2].queued ?? 0} queued`,
            },
            {
              name: "Release",
              detail: upgradeCheck
