...
function QueueDetailPage() {
    ...
    onError: (error) => {
      setMutationError(
        error instanceof Error
          ? error.message
          : "Unable to update queue configuration.",
      );
  ...
  const deliveryMutation = useMutation({
    ...
    onError: (error) =>
      feedback.failure(error, "Unable to update queue delivery."),
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Button
              variant="secondary"
              disabled={!queue.data || deliveryMutation.isPending}
              onClick={() => deliveryMutation.mutate(!paused)}
            >
      ...
      <QueueConfigDialog
        ...
        open={configOpen}
        initial={{
          ...(queue.data?.settings?.delivery_delay === undefined
            ? {}
            : { deliveryDelaySeconds: queue.data.settings.delivery_delay }),
          ...(queue.data?.settings?.message_retention_period === undefined
            ? {}
            : {
                retentionSeconds: queue.data.settings.message_retention_period,
              }),
        }}
      ...
      />
      {queue.isLoading || consumers.isLoading || metrics.isLoading ? (
        <LoadingState />
      ) : queue.error || consumers.error || metrics.error ? (
        <ErrorState message="Unable to load Queue details." />
      ) : (
        <>
          <DataTable
            columns={[
            ...
            rows={[
              {
                name: "Delivery",
                value: <StatusBadge value={paused ? "paused" : "active"} />,
              },
              {
                name: "Backlog messages",
                value: metrics.data?.backlog_count ?? 0,
              },
              {
                name: "Backlog bytes",
                value: metrics.data?.backlog_bytes ?? 0,
              },
              {
                name: "Oldest message timestamp",
                value: metrics.data?.oldest_message_timestamp_ms || "unknown",
              },
            ]}
          />
          <div className="mt-6">
