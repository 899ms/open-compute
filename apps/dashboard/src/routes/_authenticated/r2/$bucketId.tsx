...
function R2DetailPage() {
  ...
  return (
    <div>
      <PageHeader
        title={bucket.data?.name ?? bucketId}
        description="Bucket properties from the official R2 API."
      />
      {bucket.isLoading ? (
        <LoadingState />
      ) : bucket.error ? (
        <ErrorState message="Unable to load R2 bucket details." />
      ) : (
        <DataTable
          columns={[
          ...
          rows={[
            {
              property: "Created",
              value: bucket.data?.creation_date ?? "unknown",
            },
            { property: "Location", value: bucket.data?.location ?? "default" },
            {
              property: "Storage class",
              value: bucket.data?.storage_class ?? "Standard",
            },
          ]}
        />
      )}
