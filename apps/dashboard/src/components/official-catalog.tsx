...
import { DataTable, ErrorState, LoadingState, PageHeader } from "./page-layout";
...
export function OfficialCatalog({
  ...
  const createMutation = useMutation({
    ...
    onError: (error) =>
      setMutationError(
        error instanceof Error ? error.message : `Unable to create ${kind}.`,
      ),
  });
  const renameMutation = useMutation({
    ...
    onError: (error) =>
      setMutationError(
        error instanceof Error ? error.message : `Unable to rename ${kind}.`,
      ),
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) =>
      setMutationError(
        error instanceof Error ? error.message : `Unable to delete ${kind}.`,
      ),
  });
  return (
    <div>
      ...
      {create ? (
        <CreateResourceDialog
          title={`Create ${kind}`}
          description="Create this account-scoped resource through the Cloudflare v4 API."
          nameLabel="Name"
      ...
      {rename ? (
        <RenameResourceDialog
          title={`Rename ${kind}`}
          description="Update this resource through the Cloudflare v4 API."
          nameLabel="Name"
