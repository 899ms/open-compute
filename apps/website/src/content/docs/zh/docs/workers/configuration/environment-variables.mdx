...
<Tabs syncKey="package-manager">
  <TabItem label="npm">
    <Code code="npm exec -- wrangler types" lang="sh" />
  </TabItem>
  <TabItem label="pnpm">
    <Code code="pnpm exec wrangler types" lang="sh" />
  </TabItem>
  <TabItem label="bun">
    <Code code="bun run wrangler types" lang="sh" />
  </TabItem>
</Tabs>

Wrangler 会把字面量写入 `worker-configuration.d.ts`（例如 `GREETING: "Hello from TypeScript"`）。bundle 不包含 vars；开发与部署时才注入。
...
| 密钥存放位置                            | Secrets 产品                                                                                                   | [secrets](/zh/docs/workers/configuration/secrets/)，不可放入 `vars` |
