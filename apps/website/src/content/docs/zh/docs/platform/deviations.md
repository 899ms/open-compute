...
本项目按 [Workers runtime APIs](https://developers.cloudflare.com/workers/runtime-apis/) 维护已声明支持的 Worker API 子集。下列差异源于单机部署以及锁定版本的开源 `workerd`。未提供的 Cloudflare 产品见[不支持](/zh/docs/platform/unsupported/)。
...
| 静态资源                                               | 随部署存放于选定的 Local/S3 authority                                 | [静态资源](/zh/docs/workers/static-assets/)                  |
| 全球 CDN、复制、purge 传播、CDN 配额                   | 不提供                                                                |                                                              |
| Service Bindings                                       | 同一平台内的 fetch 与 RPC                                             | [Bindings](/zh/docs/workers/runtime-apis/bindings/)          |
| 跨地区放置、全球服务发现                               | 不提供                                                                |                                                              |
| 调用方准入与部署钉扎                                   | 在本机判定；失败则关闭                                                |                                                              |
| Cron                                                   | UTC 五字段，以及文档所述本机扩展                                      | [Cron 触发器](/zh/docs/workers/configuration/cron-triggers/) |
| 错过的 Cron                                            | 宽限时间内最多补最近一次，不回放停机期间的全部触发                    |                                                              |
| Cron 失败重试                                          | 按配置有限次重试；调用 `noRetry()` 除外                               |                                                              |
| Cron unknown dispatch                                  | 消耗一次 delivery attempt；只在 budget 与固定 15 分钟 deadline 内重试 |                                                              |

## 存储

| 主题                                    | 行为                                                | 文档               |
| --------------------------------------- | --------------------------------------------------- | ------------------ |
| KV                                      | 本机 SQLite；不提供全球复制或边缘缓存               | [KV](/zh/docs/kv/) |
| R2 对象                                 | 存放在选定的单节点 Local 或 S3-compatible authority | [R2](/zh/docs/r2/) |
| R2 全球就近存放 / 复制                  | 不提供                                              |                    |
| D1                                      | 本机一份 SQLite                                     | [D1](/zh/docs/d1/) |
| D1 只读副本 / 按区域路由                | 不提供                                              |                    |
...
| Workflows                               | 步骤状态位于本机 SQLite                               | [Workflows](/zh/docs/workflows/)             |
...
| 主题                                                     | 行为                                                           | 文档                                                                                        |
| -------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Cache                                                    | Workers Cache 与 Cache API 仅在本机                            | [Workers Cache](/zh/docs/workers/cache/)、[Cache API](/zh/docs/workers/runtime-apis/cache/) |
...
| 运行中的精确值                                           | `ocd capabilities --json`，见[限制](/zh/docs/platform/limits/) |                                                                                             |
| Images                                                   | 对本机请求体中的图像做变换，不是 Cloudflare 托管 Images        | [Images](/zh/docs/images/)                                                                  |
