...
5. 如有需要，`ocd` 启动 Provider，创建 socketpair，把 `OCP2`、新生成的 16 字节 nonce 和 pair 的一端发给 Provider control socket，并要求 ACK `0` + 相同 nonce。
...
`services[].props` → `ctx.props` 与 Service Binding RPC 遵循 Cloudflare [Context](https://developers.cloudflare.com/workers/runtime-apis/context/) 和 [Service Binding RPC](https://developers.cloudflare.com/workers/runtime-apis/bindings/service-bindings/rpc/)。本地扩展解析、Provider 生命周期、`OCH1`/`OCP2` 和 `HOST` 只属于 open-compute。不要把它们当成托管 Cloudflare 能力，也不要计入 Worker API inventory。
