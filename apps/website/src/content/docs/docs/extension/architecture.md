...
ocd      -- OCP2 + nonce / SCM_RIGHTS + ACK ------>  Provider
...
5. `ocd` starts the Provider if needed, creates a socketpair, sends `OCP2`, a fresh 16-byte nonce, and one end of the pair to the Provider control socket, then requires ACK `0` plus the matching nonce.
...
`services[].props` → `ctx.props` and Service Binding RPC follow Cloudflare [Context](https://developers.cloudflare.com/workers/runtime-apis/context/) and [Service Binding RPC](https://developers.cloudflare.com/workers/runtime-apis/bindings/service-bindings/rpc/). Local extension resolution, Provider lifecycle, `OCH1`/`OCP2`, and `HOST` are open-compute-only. Do not treat them as hosted Cloudflare capabilities or as members of the Worker API inventory.
