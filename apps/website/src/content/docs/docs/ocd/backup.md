...
With Local storage, a platform snapshot stored on the same disk is a consistency snapshot, not an off-host backup. The Local object root is always `<data.path>/objects`. To survive disk or host loss, stop `ocd` and independently back up the **complete instance data directory**, including its master key and `objects/format.json`, plus the actual `compute.toml`. Local fresh-host recovery restores that complete directory; `backup restore` supports S3 snapshots only. There is no Local↔S3 migration or partial-directory restore.
