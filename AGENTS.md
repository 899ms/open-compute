...
- **Day1 design for current requirements is the highest architecture rule for open-compute.** This applies to Rust, TypeScript, runtime assets, persistence, APIs, CLI, configuration, and tooling.
- Do not design for compatibility with earlier open-compute revisions, historical development data, old APIs/configuration, schemas, snapshots, artifacts, module paths, or byte/hash formats. Do not introduce or retain compatibility shims, legacy implementations, version-selection branches, dual read/write paths, aliases, defaults, or backfills to preserve those earlier states. This rule never authorizes changing a database migration that was included in a published release: those bytes may already have executed on user devices, so later schema work must append a new migration.
- Before 1.0, releases do not guarantee that all persisted data remains compatible across breaking model changes. Make such breaks explicit and fail closed; this pre-1.0 policy does not weaken the immutability of database migrations already included in a published release.
...
- The completed removals and their acceptance evidence are archived in [Day1 architecture cleanup](docs/implemented/p2-8-day1-architecture-cleanup.md). Historical stage documents do not recreate compatibility obligations.
...
- Preserve the generated `docs/implemented/g0-results.md` as the exact G0 run record; other completed documentation follows the lifecycle and retention rules in `docs/references/README.md`. Do not require the retired G0 generator to survive POC cleanup.
...
- Development Gate: exactly one round of the relevant targets per iteration; use the supported single-round commands in `docs/references/testing.md`. Never invoke a recursively chained historical entry point as a development check.
- Final Gate: `./test/gate.py --workspace` after implementation, review, fixes, static checks and coverage. Final and release acceptance require exactly one complete round; do not set `OPEN_COMPUTE_GATE_ROUNDS=3` unless the user explicitly requests a separate repetition diagnostic. See `docs/references/testing.md` and the authoritative case registry `test/gate_cases.py`.
- Final P0.1 process Gate: `OPEN_COMPUTE_TEST_WORKERD=/abs/path/to/workerd ./test/gate.py p0-1`
- Final P0.2 Worker Gate: `OPEN_COMPUTE_TEST_WORKERD=/abs/path/to/workerd ./test/gate.py p0-2`
...
- The formal multi-platform release pin is `packages/runtime/workerd.lock.json`; keep every consumer on this authoritative path. Keep one authoritative pin, not a separate POC copy. Commit the four target binaries under `share/workerd/` through Git LFS; hydrate them before building and verify their bytes against this lock. Verify target, archive checksum, binary checksum, version output, compatibility date/flags, and process flags before spawn or packaging.
- Treat a workerd pin change as a coordinated dependency update: update the formal lock and runtime assets, validate affected behavior and maintained product Gates against the formally pinned binary, record fresh evidence/API compatibility findings, and verify packaged layouts on supported hosts. For a fork, record its source revision, upstream base, build inputs, and archive/binary digests; stock workerd remains upstream comparison evidence. Historical G0 results do not prove a new pin, and targeted investigation does not require retaining or restoring the retired POC suite.
- Native Standard limits and Worker Loader development is authorized in the user's `elliothux/workerd` fork at the `third_party/workerd/` Git submodule, with the plan and source identity maintained in [workerd native runtime design](docs/workerd/README.md). Base subsequent workerd changes on that directory, preserve its independent Git state, and follow its existing component/build conventions. Commit fork changes in the submodule and record the resulting gitlink in this repository; do not treat the submodule revision as an automatic runtime pin update. Prefer existing C++ host interfaces and focused native changes; do not copy a second runtime implementation elsewhere. This replaces the former unmodified-upstream-only policy for this scope. The current formal pin remains authoritative until a coordinated fork pin update; a development binary must never bypass pin verification. Compile the checked-in Cap'n Proto configuration with the verified binary; never interpolate tenant input into Cap'n Proto source.
...
- Before a database migration is included in a published release, platform-owned SQL definitions may be revised or consolidated directly to express the current Day1 schema. Unreleased development databases do not create compatibility obligations. User-facing D1 database migrations are a separate product capability and do not change this rule.
- After a platform-owned database migration is included in a published release, never modify, remove, rename, reorder, or renumber it. Treat it as already executed on user devices and express every later schema change by appending the next contiguous migration. Preserve one authoritative current schema after migration; released-migration immutability does not justify dual read/write paths, downgrade support, silent checksum replacement, or broad compatibility shims.
...
- During implementation, review, fixes, final acceptance, and release qualification, execute each selected Gate target exactly once. Do not run three rounds, recursive historical aggregates, or duplicate an unchanged workspace run with a second aggregate. A selected scenario may still exercise multiple real-process restarts, crash points, deadlines, and recovery transitions internally. If a complete single-round entry does not exist, report that gap and use focused tests without claiming a full Gate pass. See `docs/references/testing.md`.
- Final acceptance is one complete pass after implementation, review, fixes, static checks, coverage, and source freeze. Three-round sampling is not required for any acceptance level. Address timing and concurrency risk with explicit barriers, bounded waits, deterministic fault points, and scenario-owned restart/crash coverage; use repeated diagnostics, soak, load, or fuzz only when separately requested.
- Gate entry points default to one complete round. `test/gate.py` owns selection and `test/gate_cases.py` owns the registered product-case inventory. Verify that native `--list` discovery exactly matches that registry before executing any cases; new, removed, renamed, or multiply registered cases require explicit review, never an implicit skip. Validate executed pass counts and reject ignored cases. Do not set `OPEN_COMPUTE_GATE_ROUNDS=3` unless the user explicitly requests a separate repetition diagnostic; such a run does not replace or strengthen the required single-round acceptance. Leaf tests do not read round variables, recursively invoke Gates, or hide repetition. Use `--list` for the exact plan and `--jobs` for audited process concurrency.
- Remove duplicate cases and duplicate scheduling using explicit invariant/coverage ownership. Each selected target executes once, with all selected cases in the same serial harness. Stop after a failure, preserve diagnostics, and do not retry until the cause is fixed. One-time completed capability probes are not mandatory product regression tests.
...
- Test the current Day1 model, clean initialization, and restart/crash recovery. Update or remove obsolete compatibility, upgrade, and historical byte-identity assertions with the model change while preserving security, integrity, and failure-path coverage, except that the identity and bytes of every published database migration remain frozen regression inputs. Retain regression coverage for the documented official Cloudflare API compatibility exception. Historical reports record past runs; they do not require retaining old implementations.
...
- Maintained runtime/product Gates must exercise the verified, formally pinned `workerd` (the authorized fork after its coordinated pin update), real processes, real SQLite, and the documented SigV4/network fixtures where required by the behavior under test. Do not replace Gate evidence with stock-baseline results from another revision, mocks, Miniflare, an in-memory substitute, or a skipped test.
...
- Documentation location, lifecycle, content and evidence rules are defined once in [the documentation index](docs/references/README.md).
